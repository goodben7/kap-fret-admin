...
import {
  ...
  downloadBlob,
  fetchCheckInsForManifest,
  generateCheckInManifestPdf,
  resolveCheckInManifestFlightNumber,
} from '@/lib/check-in-manifest-pdf'
import {
...
interface CheckInManifestModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function CheckInManifestModal({ open, onOpenChange }: CheckInManifestModalProps) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CheckInManifestFormData>({
    resolver: zodResolver(checkInManifestSchema),
    defaultValues: {
      departure: '',
      destination: '',
  ...
  const generate = handleSubmit(async (data) => {
    ...
    try {
      ...
      const checkIns = await fetchCheckInsForManifest(
        data.departure,
        data.destination,
        data.travelDate,
      )
      ...
      if (checkIns.length === 0) {
        toast.error('Aucun check-in trouvé pour ce trajet et cette date de vol')
        return
      }
      ...
      const flightNumber = resolveCheckInManifestFlightNumber(
        data.travelDate,
        departureCode,
        destinationCode,
        data.flightNumber,
      ...
      const blob = await generateCheckInManifestPdf({
      ...
      const nextFileName = buildCheckInManifestFileName({
        departureCode,
        destinationCode,
        travelDate: data.travelDate,
      })
      ...
      toast.success(`Manifeste généré (${checkIns.length} check-in${checkIns.length !== 1 ? 's' : ''})`)
    } catch (error) {
      if (isAxiosError(error)) {
        toast.error(extractApiErrorMessage(error.response?.data, error.response?.status))
      } else {
        toast.error('Impossible de générer le manifeste check-in')
      }
    } finally {
  ...
  return (
    <Modal
      ...
      title="Manifeste check-in"
      description="Générez le PDF paysage à partir du trajet et de la date de vol."
      className={cn(
        'rounded-2xl',
          ...
          <div className="space-y-4">
            ...
            {!canGenerate && (
              <p className="text-xs text-muted-foreground">
                Renseignez le départ, la destination et la date du vol pour activer la génération.
              </p>
            )}
          </div>
            ...
            <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-border/80 bg-muted/20">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 px-4 py-3">
                <div className="flex items-center gap-2 text-sm font-medium">
                  <UserCheck className="h-4 w-4 text-brand-orange" aria-hidden="true" />
                  Aperçu du manifeste check-in
                </div>
              ...
              <iframe
                title="Aperçu manifeste check-in"
                src={previewUrl}
